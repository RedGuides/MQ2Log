...
// The mandatory pre-setup function.  Our name is "ISXEQLog", and the class is ISXEQLog.
...
bool ISXEQLog::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQLog Loaded");
return true;
}

// shutdown sequence
void ISXEQLog::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQLogXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQLogXML);
...
void ISXEQLog::RegisterCommands()
{
    // add any commands
...
void ISXEQLog::RegisterAliases()
{
    // add any aliases
}

void ISXEQLog::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQLog::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQLog::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQLog::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQLog::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQLog::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQLogService)
    //        pISInterface->ShutdownService(this,hISXEQLogService);
}

void ISXEQLog::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQLogXML
    ISXEQLogXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQLogXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQLogXML,"General"))
    {
